...
class graphAccel:
    ...
    def plotVsTemp(self, dataframes: list[pd.DataFrame]):
        fig, axs = plt.subplots(len(dataframes), 1, figsize=(10, 12))
        
        for index, df in enumerate(dataframes):
            # Subplot for GYROscope X, Y, Z vs Time
            axs[index].scatter(df['TEMP'], df['ACCX'], label=f'{self.file_names[index][0]} GYROX', **self.scatter_style)
            axs[index].scatter(df['TEMP'], df['ACCY'], label=f'{self.file_names[index][0]} GYROY', **self.scatter_style)
            axs[index].scatter(df['TEMP'], df['ACCZ'] + 1.0, label=f'{self.file_names[index][0]} GYROZ', **self.scatter_style)
                                                    # add gravity
            axs[index].set_title('Linear Accel vs Temp')
            axs[index].set_xlabel('Temp [C]]')
            axs[index].set_ylabel('Velocity [Deg/s]')
            axs[index].legend(loc = "upper right")
        
    def plotBestFit(self, dataframes: list[pd.DataFrame]):   
        #print best fit of each axis on seperate figure
        fig, axs = plt.subplots(3, 1, figsize=(10, 12))
        axes = ["ACCX", 'ACCY', 'ACCZ']
        for index, axis in enumerate(axes):
            for df_index, df in enumerate(dataframes):
                if axis == axes[2]: # add gravity
                    df['ACCZ'] = df['ACCZ'] + 1.0
                plot_best_fit(axs[index], df['TEMP'], df[axis], f'{self.file_names[df_index][0][-6:]} {axis}')
            axs[index].set_title(f"Best Fit: Temp Vs. Accel. {axis}")
            axs[index].set_xlabel('Temp [C]]')
            axs[index].set_ylabel("Accel [G's]")
            axs[index].legend(loc = "upper right")
...
if __name__ == "__main__":
    ...
    gyro = graphAccel(file_names, None, (min_value, max_value))
    gyro.loadFiles()
    gyro.plotVsTemp(gyro.dataframes)
    gyro.plotBestFit(gyro.dataframes)
    gyro.plot()
